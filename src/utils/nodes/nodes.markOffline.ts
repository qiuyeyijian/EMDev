 ...
 *
 */
export const markOffline = async (requests: JenkinsRequests, node: string, reason: string = ""): Promise<void> => {
  const nodeRes = await get(requests, node)
  const data = { offlineMessage: reason }
  await requests.postForm(`/computer/${node}/${nodeRes.temporarilyOffline ? "changeOfflineCause" : "toggleOffline"}`, {
    ...data,
