 ...
 *
 */
export const generateToken = async (requests: JenkinsRequests, tokenName: string): Promise<JenkinsUserToken> => {
  const userName = (await whoAmI(requests)).name
  return requests
    .postForm(`/user/${userName}/descriptorByName/jenkins.security.ApiTokenProperty/generateNewToken`, {
